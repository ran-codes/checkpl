...
Modular layout with one function per file (R-style):
...
│       ├── core/
│       │   ├── __init__.py       # Re-exports verify
│       │   └── verify.py         # verify() function
│       └── predicates/
│           ├── __init__.py       # Re-exports is_uniq, not_null
...
**Why this structure?**
- Each function in its own file (easy to find and edit)
- `__init__.py` files re-export for clean user imports
- Easy to add new predicates (just add a file)
...
3. Package can be imported: `from assert_polars import verify, is_uniq, not_null`
